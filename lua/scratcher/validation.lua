...
function M.is_valid_width(width)
  ...
  return width > 0 and width < (require("scratcher.utils").is_integer(width) and vim.o.columns or 1)
end

function M.is_valid_height(height)
  if type(height) ~= "number" then return false end
  return height > 0
    and height < (require("scratcher.utils").is_integer(height) and vim.o.lines or 1)
end
